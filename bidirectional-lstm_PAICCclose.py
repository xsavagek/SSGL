...
minmax = MinMaxScaler().fit(df.iloc[:, 1:-1].astype('float32'))
df_log = minmax.transform(df.iloc[:, 1:-1].astype('float32'))
...
np.savetxt('results/bidirectional-lstm/PAICCclose/arr_true_close.csv', arr_true_close, delimiter = ',')
np.savetxt('results/bidirectional-lstm/PAICCclose/arr_pre_close.csv', arr_pre_close, delimiter = ',')
...
np.savetxt('results/bidirectional-lstm/PAICCclose/indicator.csv', indicator, delimiter = ',')
