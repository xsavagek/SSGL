...
df = pd.read_csv('./dataset/PAICCclose.csv')
date_ori = pd.to_datetime(df.iloc[:, 0]).tolist() #取第0列，转换日期为"2016-11-02"格式
print(df.head())

# 数据归一化
minmax = MinMaxScaler().fit(df.iloc[:, 1:-1].astype('float32')) #归一化，原数据-最大/最大-最小
df_log = minmax.transform(df.iloc[:, 1:-1].astype('float32')) #数据集中'Adj close'为修正后的收盘价
...
np.savetxt('results/lstm-seq2seq/PAICCclose/arr_true_close.csv', arr_true_close, delimiter = ',')
np.savetxt('results/lstm-seq2seq/PAICCclose/arr_pre_close.csv', arr_pre_close, delimiter = ',')
...
np.savetxt('results/lstm-seq2seq/PAICCclose/indicator.csv', indicator, delimiter = ',')
